...
- [x] `PaywallView.swift`: ペイウォール画面(継続的価値の明示: 写真無制限+今後の機能追加)
- [x] `AddEditPlaceView` に写真(PhotosUI)・コメント欄を追加(コメントは無料、写真はプレミアム)
...
- [ ] 審査メモにサブスクの継続的価値(写真機能、今後のアップデート計画)を記載
- [ ] 審査提出 → リジェクト対応(あれば)
...
## Developer Program 承認後にやること(一時変更の復元)

- [ ] pbxproj に `CODE_SIGN_ENTITLEMENTS = Ashiato/Ashiato.entitlements;` を戻す
- [ ] Info.plist の `AshiatoCloudEnabled` を true に戻す(entitlements復元とセットで)
  (Personal Teamでの実機テストのため一時的に外している。
   Xcodeなら Signing & Capabilities で iCloud/Background Modes を追加し直してもよい。
   entitlementsファイル自体は Ashiato/Ashiato.entitlements に残っている)
- [ ] Team を Personal Team から正式な Individual Team に切り替え
